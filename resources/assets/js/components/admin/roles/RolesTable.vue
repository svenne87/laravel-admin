<template>
    <div class="ui container table-responsive">
        ...
        <button class="btn btn-primary float-left" @click="createRole()">{{ $t('general.create_role') }}</button>
        ...
        >
            <template slot="icon" slot-scope="props">
                <i class="icon-organization mr-1" v-bind:style="{color: '#636b6f'}"></i>
            </template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            editRow(rowData){
                this.$router.push({ path: `/roles/${rowData.id}`})
            },
            deleteRow(rowData){
                this.show = false;
                this.currentRowData = {};
                ...
                axios.delete('/api/v1/roles/' + rowData.id)
